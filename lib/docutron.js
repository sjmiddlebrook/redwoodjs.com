...
const convertToHtml = (pages, book, version, githubPath) => {
...
const create = (markdown, book, options = {}) => {
  ...
  let githubPath;
  if (options.hasOwnProperty('githubPath')) {
    githubPath = options.githubPath
  } else if (options.hasOwnProperty('file')) {
    githubPath = `https://github.com/redwoodjs/redwoodjs.com/blob/main${options.file.substr(1)}`
  } else {
    githubPath = `https://github.com/redwoodjs/redwoodjs.com/blob/main${options.url.substr(1)}`
  }
  const htmlPages = convertToHtml(markdownPages, book, options.version, githubPath)
